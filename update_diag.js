...
const path = 'astro-web/src/data/diagnosticoData.ts';
...
if (!content.includes('PLATFORM_AXIS_ORDER')) {
  content += '\n' + orderConst;
}
...
const customWhys = {
  ...
  'c_contenido_externo': 'Excluir a la red externa frena la calidad de la cadena de valor o los ingresos adicionales. → conviene priorizar Integración Externa/Cert.',
...
let matchCount = 0;
// We look for id: 'something', ... why: 'El contexto...' and replace 'El contexto...'
content = content.replace(/id:\s*'([^']+)',([\s\S]*?)why:\s*'El contexto operativo detrás de esta fricción afecta directamente a la escalabilidad de tus procesos B2B\.'/g, (match, id, between) => {
  if (customWhys[id]) {
    matchCount++;
    return \`id: '\${id}',\${between}why: '\${customWhys[id]}'\`;
  }
  return match;
});

fs.writeFileSync(path, content, 'utf8');
console.log('Replaced custom whys:', matchCount);
