...
(() => {
	...
	if (!activeSection) {
		...
		if (solucionesKeywords.some((k) => urlPath.indexOf(k) !== -1)) {
			activeSection = "soluciones";
		} else if (recursosKeywords.some((k) => urlPath.indexOf(k) !== -1)) {
	...
	document.querySelectorAll("header a[href]").forEach((a) => {
		...
		if (
			...
			href.indexOf("tel:") === 0 ||
			href.indexOf("http") === 0 ||
			a.classList.contains("logo-link") ||
			a.classList.contains("btn-cta")
		)
			return;
...
function toggleMob(sectionId, btnId) {
	...
	btn.setAttribute(
		"aria-expanded",
		btn.classList.contains("open") ? "true" : "false",
	);
}
