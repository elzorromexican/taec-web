...
async function run() {
	const { supabase, EMBEDDING_DIMENSION, searchSimilarChunks } = await import(
		"../src/lib/tito/rag.js"
	);
	...
	console.log(
		"Testeando similarity con pgvector usando query genérico (todo 0.1) ...",
	);
	...
	if (searchResults.length === 0) {
		console.error(
			"Fallo. La búsqueda devolvió 0 resultados. ¿Embeddings inválidos?",
		);
		allGood = 0;
	} else {
		for (const r of searchResults) {
			if (Number.isNaN(r.similarity) || r.similarity === null) {
				console.error(
					`ERROR CRITICO: similarity es NaN o nulo para id ${r.id}`,
				);
				allGood = 0;
			}
		}
	...
	if (allGood) {
		console.log(
			"✅ HEALTH CHECK APROBADO: No se detectaron NaNs, dimensión es de 768.",
		);
