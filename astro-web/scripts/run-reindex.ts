...
async function run() {
	const { supabase, getEmbedding, indexKnowledgeChunk } = await import(
		"../src/lib/tito/rag.js"
	);
	...
	const { error: deleteError } = await supabase
		...
		.delete()
		.neq("id", "00000000-0000-0000-0000-000000000000"); // Clean everything
	...
	for (const pathname of SEED_URLS) {
		...
		try {
			...
			if (!response.ok) {
				console.warn(
					`WARN: Status ${response.status} en ${fullUrl}. Saltando.`,
				);
				continue;
			}
			...
			for (const chunk of chunks) {
				try {
					...
					console.error(
						`Error generando/insertando chunk para ${pathname}:`,
						embErr,
					);
			...
			console.log(
				`Indexación completada para ${pathname}. Chunks: ${successChunks}`,
			);
