...
export function extractSemanticChunks(
	...
	$(
		'nav, header, footer, script, style, noscript, svg, iframe, [id^="bar-promo"]',
	).remove();
	...
	$main.find("*").each((i, el) => {
		...
		if ($(el).is("h1, h2, h3, h4")) {
			...
			if (currentSectionTitle.toLowerCase().includes("precio"))
				currentIntentHint = "pricing";
			else if (
				currentSectionTitle.toLowerCase().includes("faq") ||
				currentSectionTitle.toLowerCase().includes("preguntas")
			)
				currentIntentHint = "faq";
			else if (
				currentSectionTitle.toLowerCase().includes("feature") ||
				currentSectionTitle.toLowerCase().includes("beneficio")
			)
				currentIntentHint = "features";
		...
		if ($(el).is("details")) {
			...
			if (content.length > 20) {
				chunks.push({
					text: `FAQ: ${summary} - Respuesta: ${content}`,
		...
		if ($(el).is("table")) {
			...
			if (tableText.length > 20) {
				chunks.push({
					text: `Tabla/Comparativo: ${tableText}`,
					sectionTitle: currentSectionTitle,
					intentHint: currentIntentHint,
				});
			}
	...
	for (const c of chunks) {
		if (
			c.sectionTitle !== lastSectionTitle ||
			c.intentHint !== lastIntentHint ||
			currentGroupText.length > 1000
		) {
			if (currentGroupText.trim() !== "") {
				const hash = crypto
					.createHash("sha256")
					.update(currentGroupText)
					.digest("hex");
				bundledChunks.push({
	...
	(async () => {
		...
		for (const u of URLS_TEST) {
			...
			const chunks = extractSemanticChunks(
				html,
				u.replace("https://stellar-mermaid-3ba7f1.netlify.app", ""),
			);
