...
import { PLATFORM_AXIS_ORDER, stages } from "../../data/diagnosticoData";
import {
	calcularDiagnostico,
	type DiagnosticResult,
} from "../../lib/diagnostico/diagnosticoScoring";
...
export default function DiagnosticoApp() {
	...
	const currentStage = useMemo(
		() => stages.find((s) => s.id === session.stage) || null,
		[session.stage],
	);
	...
	const isCurrentStageComplete = useMemo(() => {
		...
		return currentStage.questions.every(
	...
	const handleEmailSubmit = async (e: React.FormEvent) => {
		...
		try {
			...
			if (!apiRes.ok) {
				alert(
					"Error conectando al sistema. Por favor revisa los datos e intenta de nuevo.",
				);
				return;
			}

			// Extraer todas las respuestas para análisis crítico
			const fullQA = Object.entries(session.answers)
				.map(([id, val]) => {
					const q = currentStage.questions.find((x) => x.id === id);
					const intensity =
						val === 2 ? "CRÍTICO" : val === 1 ? "MODERADO" : "CONTROLADO";
					return `[${intensity}] - ${q?.text}`;
				})
				.join("\n");

			const sortedPlats = PLATFORM_AXIS_ORDER.map((p) => ({
				id: p,
				score: res.platformScores[p] || 0,
...
- IMPORTANTE: TIENES ESTRICTAMENTE PROHIBIDO volver a pedir su correo, teléfono o nombre en esta sesión. Oblígate a mantener el diálogo profundo sobre sus problemas y arquitectura.`;
			...
			window.dispatchEvent(
				new CustomEvent("OpenTitoDiagnostic", {
					detail: {
						email: session.email,
						diagnosticResult: res.winningPlatform,
						prompt,
					},
				}),
			);
        ...
        `}</style>
				...
				>
					...
					>
						{session.stage && (
							<button
								...
								style={{
									...
									border: "1px solid rgba(128, 221, 213, 0.4)",
									color: "#80DDD5",
									padding: "4px 8px",
									borderRadius: "4px",
									fontSize: "11px",
						...
						<span>
							Etapa: {currentStage ? currentStage.label : "Sin seleccionar"}
						</span>
					</div>
				</div>
				...
				<div style={{ maxWidth: "900px", margin: "0 auto", padding: "2rem" }}>
					{session.phase === "p0" && (
						...
						>
							...
							>
								Para ofrecerte un diagnóstico preciso y contextualizado,
								necesitamos clasificar la madurez y alcance inicial de tus
								necesidades de digitalización de conocimiento.
							</p>
							...
							>
								{stages.map((st) => (
									<div
										...
										onMouseEnter={(e) =>
											(e.currentTarget.style.borderColor = "#0A7A70")
										}
										onMouseLeave={(e) =>
						...
						<div style={{ animation: "fadeSlideIn 0.5s ease" }}>
							...
							>
								<div>
									...
									>
										Diagnóstico de {currentStage.label}
									</div>
								</div>
							</div>
								...
								{currentStage.questions.map((q, qi) => {
									...
									return (
										<div
											...
											onClick={() =>
												isAnswered &&
												setActiveInsight(activeInsight === q.id ? null : q.id)
											}
											style={{
											...
											>
												<div
													style={{
														...
														fontSize: "12px",
														fontWeight: 500,
														color: isAnswered ? "#0A7A70" : "#C8C8D4",
														flexShrink: 0,
														minWidth: "28px",
														marginTop: "2px",
													}}
												...
												<div style={{ flex: "1 1 250px" }}>
													<div
														style={{
															fontSize: "15px",
															fontWeight: 400,
															color: isAnswered ? "#0A7A70" : "#1B2A4A",
															lineHeight: 1.4,
															marginBottom: "6px",
														}}
													>
													...
													{[0, 1, 2].map((level) => {
														...
														return (
															<button
																...
																onClick={(e) => {
																	e.stopPropagation();
																	handleLevelSelect(q.id, level as PainLevel);
																}}
																style={{
																	padding: "8px 12px",
																	borderRadius: "4px",
					...
					{session.phase === "email_gate" && (
						...
						>
							...
							>
								Último Paso para ver tus Resultados
							</h2>
							<p
								style={{
									fontSize: "16px",
							...
							>
								Hemos consolidado tu matriz operativa en base a las
								vulnerabilidades reportadas. Ingresa tu correo corporativo para
								liberar la arquitectura recomendada.
							</p>
							...
							>
								...
								>
									{isSending
										...
										: "Ver Diagnóstico Arquitectónico →"}
								</button>
							</form>
						</div>
					)}
							...
							>
								...
								>
									{(() => {
										const sortedPlats = PLATFORM_AXIS_ORDER.map((p) => ({
											id: p,
											score: session.result!.platformScores[p] || 0,
										})).sort((a, b) => b.score - a.score);
										const motorInicial = sortedPlats[0];
										const segundaCapa =
											sortedPlats[1]?.score >= 40 ? sortedPlats[1] : null;
										const evolucion =
											sortedPlats[2]?.score >= 25 ? sortedPlats[2] : null;

										return (
											<>
												<div>
													Motor inicial ·{" "}
													<strong style={{ color: "#0A7A70" }}>
														{platformLabels[motorInicial.id]}
													</strong>
												</div>
												{segundaCapa && (
													<div>
								...
								>
									Tu operación requiere intervenciones prioritarias en:{" "}
									<b>
										{session.result.topPains
											.map((p) => painLabels[p as PainAxis])
											.join(", ")}
									</b>
									.
								</p>
								...
								>
										...
										<div style={{ height: "250px" }}>
											<ResponsiveContainer width="100%" height="100%">
												<RadarChart
													...
													cy="50%"
													outerRadius="70%"
													data={Object.entries(session.result.painProfile).map(
														([axis, score]) => ({
									...
									>
										...
										>
											Alineación Arquitectónica (Plataformas)
										</h3>
										<div style={{ height: "250px" }}>
											<ResponsiveContainer width="100%" height="100%">
												<RadarChart
													...
													cy="50%"
													outerRadius="70%"
													data={PLATFORM_AXIS_ORDER.map((plat) => ({
								...
								>
									...
									<div>
										...
										>
											Tito Bits ya recibió tu expediente
										</h3>
										<p
											style={{
												fontSize: "14px",
										...
										>
											Nuestro arquitecto virtual ha asimilado tus puntos
											críticos de dolor y la tecnología ganadora. La ventana
											emergente en la <b>esquina inferior derecha</b> ya está
											activa.{" "}
											<b>
												Escríbele para revisar a profundidad tu mapa de riesgo.
											</b>
										</p>
									</div>
								</div>
