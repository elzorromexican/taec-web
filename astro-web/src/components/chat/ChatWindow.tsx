		...
		<>
			...
			>
				{!isOpen && (
					<div
						style={{
							...
							background:
								userData.countryCode === "MX"
									? "var(--check-green)"
									: "#F59E0B",
						}}
					>
						{hasUnread
							? "¡Tito tiene un mensaje nuevo! 💬"
							: userData.countryCode === "MX"
								? "¡Promo MX Desbloqueada! 🇲🇽"
								: "Diagnóstico de e-learning ⚡"}
					</div>
				)}
				<style
				...
				{!isOpen ? (
					<svg
						viewBox="0 0 100 100"
						style={{ width: "38px", height: "38px", fill: "white" }}
					>
						<circle
							cx="50"
							cy="40"
					...
					<div style={chatStyles.headerContainer}>
						<div style={{ display: "flex", alignItems: "center", gap: "12px" }}>
							<div style={chatStyles.headerIconWrapper}>
								<svg
									viewBox="0 0 100 100"
									style={{ width: "28px", height: "28px", fill: "#004775" }}
								>
									<circle
										cx="50"
										cy="40"
						...
						{!hasStarted ? (
							<div style={chatStyles.startScreenContainer}>
								<div style={chatStyles.startScreenTextContainer}>
									...
									<p style={chatStyles.startScreenSubtitle}>
										Yo soy Tito Bits. ¿Con quién tengo el gusto?
									</p>
								</div>
							...
							<>
								{messages
									.filter(
										(m: any) => !m.text.includes("[SYSTEM_HIDDEN_CONTEXT]"),
									)
									.map((m: any, i) => (
										<div
											key={i}
													...
													<div className="react-markdown-container">
														...
														{m.promo && (
															<div style={chatStyles.promoContainer}>
																<div style={chatStyles.promoBadge}>
																	{m.promo.badgeText}
																</div>
																<h4 style={chatStyles.promoTitle}>
																	{m.promo.title}
																</h4>
																<p style={chatStyles.promoDescription}>
																	{m.promo.description}
																</p>
																{m.promo.link && (
																	<a
														...
														{isRolloutActive &&
															...
															i ===
																messages.filter(
																	(x: any) =>
																		!x.text.includes("[SYSTEM_HIDDEN_CONTEXT]"),
																).length -
																	1 && (
																<button
																	onClick={(e) => {
					...
					{hasStarted && (
						<div style={chatStyles.footerContainer}>
								...
								<form onSubmit={sendMessage} style={chatStyles.footerForm}>
									...
									<button
										...
										style={{
											...chatStyles.footerSubmitButton,
											background:
												!input.trim() || isLoading ? "#D1D5DB" : "#F59E0B",
											cursor:
							...
							<div style={chatStyles.footerDisclaimer}>
								Tito Bits es una IA en entrenamiento y puede cometer errores.
								Por favor, consulta cualquier duda técnica o financiera con tu
								Asesor de Ventas.
							</div>
						</div>
					)}
