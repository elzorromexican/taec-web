...
export default function CurrencyCalculator() {
  ...
  const formatC = (val: number, c: string) => new Intl.NumberFormat('es-MX', { style: 'currency', currency: c }).format(val);
  const currentUsd = typeof usdAmount === 'number' ? usdAmount : 0;

  return (
    <div style={{ fontFamily: 'Inter, system-ui, sans-serif', fontSize: '12px', background: '#f8fafc', padding: '10px', borderRadius: '6px', border: '1px solid #e2e8f0', marginTop: '10px' }}>
      
      {/* Input USD - Max 14px */}
      <div style={{ marginBottom: '10px', background: '#fff', border: '1px solid #cbd5e1', borderRadius: '4px', padding: '4px 8px', display: 'flex', alignItems: 'center' }}>
        <span style={{ color: '#475569', fontWeight: 600, marginRight: '4px', fontSize: '13px' }}>USD $</span>
        <input 
          type="number" 
          value={usdAmount} 
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: '8px', padding: '0 4px' }}>
        <a href="https://www.banxico.org.mx/tipcamb/main.do?page=tip&idioma=sp" target="_blank" rel="noopener noreferrer" style={{ color: '#64748b', textDecoration: 'none', fontSize: '10px', fontWeight: 600 }}>Auditar Banxico ↗</a>
        <a href="https://suameca.banrep.gov.co/estadisticas-economicas/informacionSerie/1/tasa_cambio_peso_colombiano_trm_dolar_usd" target="_blank" rel="noopener noreferrer" style={{ color: '#64748b', textDecoration: 'none', fontSize: '10px', fontWeight: 600 }}>Auditar Banrep ↗</a>
      </div>
    </div>
  );
}
