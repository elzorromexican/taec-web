...
export function calcularScore(signals: LeadSignals): number {
  ...
  const getConfidence = (field: string) => {
    if (signals.confidence_scores && signals.confidence_scores[field] !== undefined) {
  ...
  if (signals.requiere_integracion) score += (15 * getConfidence('requiere_integracion'));

  // Madurez de ciclo de vida
  if (signals.presupuesto_aprobado) score += (25 * getConfidence('presupuesto_aprobado'));
