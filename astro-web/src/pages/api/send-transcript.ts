...
export const POST: APIRoute = async ({ request }) => {
	try {
		...
		if (!rawBody || rawBody.length > 100000) {
			// Max ~100KB transcript
			return new Response(
				JSON.stringify({
					error: "Payload body vacío o excede el límite permitido.",
				}),
				{ status: 413 },
			);
		}
		...
		if (!Array.isArray(messages) || messages.length > 50) {
			return new Response(
				JSON.stringify({ error: "Formato de historial inválido o excesivo." }),
				{ status: 400 },
			);
		}
		...
		const parseBasicMarkdown = (str: string) => {
			...
			html = html.replace(
				/\[([^\]]+)\]\((https?:\/\/[^\s)]+)\)/g,
				'<a href="$2" style="color:#004775;text-decoration:underline;">$1</a>',
			);
		...
		if (!resendKey) {
			console.error(
				"ALERTA DE INFRAESTRUCTURA (Netlify): resendKey undefined. La llave secreta de correo no fue inyectada en el entorno.",
			);
			return new Response(
				JSON.stringify({
		...
		const messagesHtml = messages
			.map((m: any) => {
				...
				return (
					...
					'<div style="' +
					marginClass +
					" max-width: 80%; padding: 12px; border-radius: 8px; background-color: " +
					bgColor +
					"; border: 1px solid " +
					borderColor +
					'; text-align: left;">' +
					...
					nameColor +
					';">' +
					parseBasicMarkdown(senderName) +
					"</strong><br/>" +
					'<span style="font-size: 14px; line-height: 1.5; color: #333;">' +
					parseBasicMarkdown(m.text) +
					"</span>" +
					"</div>" +
					"</div>"
				);
		...
		const emailHtml =
			...
			escapeHtml(userData.phone || "No provisto") +
			"</p>" +
			"<p><strong>Email Institucional:</strong> " +
			escapeHtml(userData.email) +
			"</p>" +
			"<p><strong>Fecha/Hora:</strong> " +
			escapeHtml(metadata?.timestamp || new Date().toISOString()) +
			"</p>" +
			"<p><strong>Página:</strong> " +
			escapeHtml(metadata?.url || "Desconocida") +
			"</p>" +
		...
		const { data, error } = await resend.emails.send({
			...
			to: [
				(process.env.TRANSCRIPT_RECIPIENT_EMAIL ||
					process.env.VENTAS_EMAIL) as string,
			],
		...
		if (error) {
			...
			return new Response(
				JSON.stringify({ error: "Fallo al despachar la notificación." }),
				{ status: 400 },
			);
		}
