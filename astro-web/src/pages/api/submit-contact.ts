...
export const POST: APIRoute = async ({ request }) => {
  ...
  try {
    ...
    if (!clientEmail || !privateKey || !sheetId) {
      console.error(`[API Contacto] Faltan variables de entorno para Google Sheets. TX_ID: ${transactionId}`);
    ...
    return new Response(JSON.stringify({ success: true, tx_id: transactionId }), { 
      status: 200, 
    ...
    console.error(`[CATCH API Contacto] Error procesando el guardado en Sheets. TX_ID: ${transactionId} | Detalles:`, errorMsg);
    ...
    if (d) { 
      ...
      if (resendKey) {
         try {
           ...
           const escapeHtml = (str: string) => String(str).replace(/[&<>"']/g, match => {
             const escape: any = { '&': '&amp;', '<': '&lt;', '>': '&gt;', '"': '&quot;', "'": '&#039;' };
             return escape[match];
           });
           ...
           console.log(`[Backup Enviado] El lead ${d.nombre} fue rescatado y enviado por Resend exitosamente. TX_ID: ${transactionId}`);
         } catch (resendError) {
           console.error(`[CRÍTICO] Falló Google Sheets Y Falló Resend. Lead en riesgo. TX_ID: ${transactionId}`, resendError);
         }
      } else {
        console.error(`[CRÍTICO] No se encontró RESEND_API_KEY para ejecutar el Backup del Lead. TX_ID: ${transactionId}`);
      }
    }

    // Disparamos Status 500 para activar el catch del frontend, pero devolvemos una Ref para soporte
    return new Response(JSON.stringify({ error: `Hubo un error interno al conectar con el CRM de reportes. Ref: ${transactionId}` }), { 
      status: 500, 
      headers: { 'Content-Type': 'application/json' } 
    });
  }
