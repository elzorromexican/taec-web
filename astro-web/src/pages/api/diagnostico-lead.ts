	...
	try {
		...
		if (!success) {
			return new Response(
				JSON.stringify({
					error:
						"Demasiadas consultas en corto tiempo. Intenta de nuevo en unos minutos.",
				}),
		...
		if (!rawBody || rawBody.length > 12000) {
			return new Response(
				JSON.stringify({ error: "Payload body vacío o demasiado grande." }),
				{ status: 413 },
			);
		}
		...
		if (
			!platformScores ||
			typeof platformScores !== "object" ||
			!painProfile ||
			typeof painProfile !== "object" ||
			!winningPlatform
		) {
		...
		for (const [k, v] of Object.entries(platformScores)) {
			...
			if (!isFinite(pScore) || pScore < 0 || pScore > 100) {
				return new Response(
					JSON.stringify({
						error: "Valor numérico inválido en platformScores.",
					}),
					{ status: 400 },
				);
			}
		...
		for (const [k, v] of Object.entries(painProfile)) {
			...
			if (!isFinite(pScore) || pScore < 0 || pScore > 100) {
				return new Response(
					JSON.stringify({ error: "Valor numérico inválido en painProfile." }),
					{ status: 400 },
				);
			}
		...
		const sortedPlats = PLATFORM_AXIS_ORDER.map((p) => ({
			id: p as string,
			score: platformScores[p as keyof typeof platformScores] || 0,
		...
		if (!resendKey) {
			return new Response(
				JSON.stringify({ error: "No Resend API Key found" }),
				{ status: 500 },
			);
		}
		...
		const emailHtml =
			...
			"<p><strong>Email Institucional:</strong> " +
			escapeHtml(email) +
			"</p>" +
			"<p><strong>Etapa Operativa:</strong> " +
			escapeHtml(stage) +
			"</p>" +
			"<p><strong>Índice de Urgencia (Pains):</strong> " +
			safeUrgency +
			"%</p>" +
			'<p><strong>Plataforma Resultante:</strong> <span style="color:#D95A1E; font-weight:bold;">' +
			...
			"%</li>" +
			"<li>Contenido: " +
			(painProfile.contenido || 0) +
			"%</li>" +
			"<li>Tecnología: " +
			(painProfile.tecnologia || 0) +
			"%</li>" +
			"<li>Normativa: " +
			(painProfile.normativa || 0) +
			"%</li>" +
			"<li>Escala: " +
			(painProfile.escala || 0) +
			"%</li>" +
			"<li>Monetización: " +
			(painProfile.monetizacion || 0) +
			"%</li>" +
			"</ul>" +
		...
		if (error) {
			...
			return new Response(
				JSON.stringify({
					error: "Fallo al despachar la notificación interna.",
				}),
				{ status: 400 },
			);
		}
		...
		const axisDescriptions = {
			lms_corp:
				"Requiere un motor robusto para mapear el organigrama y automatizar el plan de carrera de empleados.",
			lms_agil:
				"Prioridad en inmediatez: necesita subir contenido en minutos para consumo preferentemente móvil.",
			fabrica_ddc:
				"Sus cursos necesitan transformarse para cumplir urgentemente con normativas oficiales y atrapar al alumno.",
			lms_cert:
				"Requiere auditoría externa, certificados oficiales y las rutas de aprendizaje son obligatorias.",
			eval_proctor:
				"El valor de sus cursos reside en la seguridad. Evitar suplantaciones y trampas es vital.",
			vilt_zoom:
				"Aprendizaje presencial/digital mixto. La gestión de cohortes (Zoom/Teams) es fundamental.",
			ecommerce:
				"Proyecto pensado para monetizar. Requiere pasarelas de pago y la promoción es vital (B2C/B2B2C).",
			tools_autor:
				"Su equipo requiere autonomía para diseñar e iterar contenido rápidamente sin depender de externos.",
		};
      ...
      `,
		...
		const quickChartUrl =
			`https://quickchart.io/chart?width=500&height=400&c=` +
			encodeURIComponent(
				JSON.stringify({
					type: "radar",
					data: {
      ...
      <div style="max-width: 600px; background: white; margin: auto; border: 1px solid #E5E7EB; border-radius: 12px; overflow: hidden; box-shadow: 0 4px 20px rgba(0,0,0,0.05);">
		...
		try {
			...
			console.warn(
				"Fallo el envío al prospecto (posible barrera Sandbox de Resend):",
				prospectErr,
			);
		}
		...
		console.error(
			"Crash local generando o enviando email (Diagnóstico):",
			error,
		);
		return new Response(
