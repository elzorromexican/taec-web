...
- [x] **[SPRINT-03-MAY-2026] 🔴 SEO Alta:** Actualización de Meta-Tags y Títulos de producto (<150 chars) e inyección de CTAs (Issue #213). Alineación y corrección de etiquetas `<h1>` ocultas con `<meta>` titles (Issue #214).
- [x] **[SPRINT-03-MAY-2026] 🟡 SEO Media:** Enriquecimiento de la meta descripción del Home Page exaltando partners oficiales (Issue #215). Truncado de 18 Page Titles para prever SERP truncation (Issue #216). Remoción estratégica de etiqueta hreflang en espera de internacionalización completa (Issue #217). Auditoría profunda de `/articulos/` aplicando `noindex` o redirects 410 a contenido obsoleto o de baja calidad (Issue #218).
...
- [x] **Validación Estructural del Tarifario DDC:** *RESUELTO POR DEPRECIACIÓN*. Se erradicó la dependencia local del `ddc-pricing-matrix.json` mudando la lógica a la app B2B externa y descomisionando el API interno (`quote-ddc.ts`) en la bóveda `/docs/legacy-ddc-logic/`.
- [x] **Monitoreo de Resiliencia en Contacto:** Inyectada telemetría (UUID) y un patón `Promise.race()` de 5s en `/api/submit-contact.ts`. Las caídas directas de Google Sheets ahora disparan un fallback estricto que rescata el Lead a la bandeja DevOps mediante `Resend`.
...
- [x] **Single Source of Truth Comercial (Pricing):** *ARCHIVADO*. La deuda técnica por bifurcación de lógicas fue erradicada físicamente del stack local. La única métrica de verdad de precios recae hoy en el cotizador app externo.
- [x] **Logging Operativo en Nodos Core:** Trazas analíticas inyectadas en todos los try/catch críticos (`submit-contact` y `send-transcript`). Correlación implementada vía *Transaction IDs* atados a los payloads de emergencia.
